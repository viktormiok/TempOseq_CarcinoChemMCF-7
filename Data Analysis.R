...
names(colData) <- c("well_ID", "lane", "chemical", "concentration", "estrogen", 
                    "biological_replicate", "plate_assignment","technical_replicate",
                    "plate"
)
...
m_param = mcols(dds)
med_residual_param <- median(abs(log(m_param$dispGeneEst) - log(m_param$dispFit)), 
                             na.rm = TRUE
)
dds <- estimateDispersions(dds, 
                           fitType="local"
...
med_residual_local = median(abs(log(m_local$dispGeneEst) - log(m_local$dispFit)), 
                            na.rm = TRUE
)
```
...
counts_median$merge_criterion <- sapply(strsplit(row.names(counts_median),"_"),
                                        function(x) x[1])
tmp <- merge(counts_median,
             characteristic.DF, 
...
We have included two displays of the PCA. 
```{r Plot PCA of all samples}
idx = rowMeans(counts(dds, normalized=TRUE)) >= 5
ddsFiltered = dds[idx,]
vsd <- vst(ddsFiltered, 
           blind=TRUE,
           nsub=nrow(ddsFiltered), 
           fitType="local"
)
pcaData <- plotPCA(vsd, 
                   intgroup=c("estrogen", "chemical"),
                   returnData=TRUE
)
...
vsd_E2 <- vst(ddsFiltered_E2treated,
              blind=TRUE,
              nsub=nrow(ddsFiltered_E2treated),
              fitType="local"
)
pcaData_E2treated <- plotPCA(vsd_E2, 
...
The variables ```lane``` and ```plate``` are perfectly correlated, so we would not include ```lane``` in our design formula. The variable ```plate_assignment``` contains information about ```chemical``` (because there are only three levels of ```chemical``` for each ```plate assignment```. 
...
for (j in 1:3){
  ...
  for (i in 1:length(chemicals)){
    ...
    chem_intersect_len = length(base::intersect(diff_expressed_genes,
                                                diff_expressed_genes_estrogen_starved_versus_stimulated)
    )
...
dds_E2 <- estimateDispersions(dds_E2,
                              fit='local'
)
...
for (j in 1:3){
  ...
  chemicals_vs_control_E2_stimulated = data.frame(chemicals_vs_control_E2_stimulated,
                                                  column_values
  )
}
...
CrossTable <-matrix(c(24, 8, 272, 198),
                    nrow=2,
...
pathwayBarPlot <- function(finTab){
  ...
  type = c(rep("Significant genes", nrow(finTab)),
           rep("No significant genes", nrow(finTab)), 
...
for(i in 1:14){
  ...
  CrossTab = matrix(c(nrow(tmp[which(tmp[,3] == 1),]), 
                       ...
                       nrow(tmp1[which(tmp1[,3] == 0),])),
                    nrow=2
  )
  fin[i,4] <- fisher.test(CrossTab, 
                          alternative="greater")$p.value
}
fin$X5 <- p.adjust(fin[,4], 
                   method="fdr"
)
colnames(fin) = c("Characteristics", "CharactSize",
                   "NrSigGen", "Pval","adjPval")
fin <- fin[order(fin[,5], fin[,4], decreasing=FALSE),]
```
...
CrossTable <- matrix(c(47, 23, 237, 168),
                     nrow=2,
                     dimnames=list(c("Diff. Expressed", "Not Diff. Expressed"),
...
df$Characteristics <- factor(df$Characteristics, 
                             levels=unique(df$Characteristics[idx])
)
