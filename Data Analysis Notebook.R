...
```{r}
colData_raw <-  read.csv("Condition_table_detailed.csv", header=TRUE)
...
colData$estrogen <- relevel(colData$estrogen, ref = "starved")
colData$chem_dose <- relevel(colData$chem_dose, ref = "control_0")
```
...
plates_all <- join_all(list(PL1A_raw,PL1B_raw,PL1C_raw,PL1D_raw,
                                ...
                                PL3A_raw,PL3B_raw,PL3C_raw,PL3D_raw), by = "Gene", type="full")
...
aux <- data.frame(Well = colnames(countData_matrix), Count = colSums2(countData_matrix))
aux$Chemical <- sapply(aux$Well, function(x) colData[x,"chemical"])
aux$Dose <- sapply(aux$Well, function(x) colData[x,"concentration"])
aux$Estrogen <- sapply(aux$Well, function(x) colData[x,"estrogen"])
...
tmp <- head(aux[order(aux$`Total count`), ], n = 10)
tmp[order(tmp$Well),]
```
...
dds <- collapseReplicates(dds, dds$mega_var, renameCols=TRUE)
...
sprintf("Median residual of parametric fit: %f" , med_residual_param)
sprintf("Median residual of local fit: %f" , med_residual_local)
```
...
hist(sizeFactors(dds), xlim=c(0,3))
```


```{r}
characteristic.DF <- read.xls(xls = '/Users/viktorian/Documents/SilentSpring/SilentSpring Data/characteristics_table.xlsx', sheet=2)
characteristic.DF <- data.frame(characteristic.DF[,0:19], row.names = 'Gene.symbol')
```
...
f <- function(x, counts) {
 gene_name <- x[1]
 characteristic <- x[4]
 paste(gene_name,characteristic, sep='\t')
}

counts_norm <- counts(dds, normalized=TRUE)
median_counts <- apply(counts_norm, 1, median)
...
tmp <- merge(counts_median, characteristic.DF, by.x='merge_criterion', by.y='row.names')
...
pcaData <- merge(x = pcaData, y = as(colData(dds),'data.frame')[, c("concentration", "mega_var")], by.x='row.names', by.y='mega_var')
...
vsd_noE2 <- vst(ddsFiltered_E2untreated, blind = TRUE, nsub = nrow(ddsFiltered_E2untreated), fitType="local")
pcaData_E2untreated <- plotPCA(vsd_noE2, intgroup=c("chemical", "concentration"), returnData=TRUE)
...
col_ann <- data.frame(estrogen = colData(dds)$estrogen, chemical = colData(dds)$chemical)
rownames(col_ann) <- colnames(sampleDistMatrix)

# List with colors for each annotation.
ann_colors <- list(estrogen = brewer.pal(2, "Set3"), chemical = brewer.pal(9, "Set1"))
...
vst.data0 <- varianceStabilizingTransformation(dds_noE2, blind = TRUE, fitType = "local")
vst.data1 <- varianceStabilizingTransformation(dds_E2, blind = TRUE, fitType = "local")
...
# Data frame with column annotations.
col_ann0 <- data.frame(chemical = colData(dds_noE2)$chemical, dose = factor(colData(dds_noE2)$concentration))
rownames(col_ann0) <- colnames(sampleDistMatrix0)

col_ann1 <- data.frame(chemical = colData(dds_E2)$chemical, dose = factor(colData(dds_E2)$concentration))
rownames(col_ann1) <- colnames(sampleDistMatrix1)

# List with colors for each annotation.
ann_colors <- list(chemical = brewer.pal(9, "Set1"), dose = brewer.pal(5, "Set2"))
...
res_estrogen_starved_versus_stimulated <- results(dds, contrast = c("estrogen","starved","stimulated"))
...
hist(res_estrogen_starved_versus_stimulated$padj, main = "Histogram of p-values", xlab = "Adjusted p-values", breaks = 20)
```
...
dds_noE2 <- estimateDispersions(dds_noE2, fit='local')
...
for (j in 1:3){
  ...
  for (i in 1:length(chemicals)){
    ...
    # getting result
    number_of_diff_expressed_genes = sum(res_control_vs_chem$padj < 0.05, na.rm = T)
  ...
  chemicals_vs_control_E2_starved = data.frame(chemicals_vs_control_E2_starved, column_values)
}
...
upset(fromList(diff_expressed_genes_control_vs_chem_noE2), order.by = 'freq', sets = c("BBP vs Control","BPA vs Control", "Gen vs Control", "Tam vs Control", "Estrogen_stimulated vs Estrogen_starved"))
...
```{r}
ddsLRT <- nbinomLRT(dds, full = ~ biological_replicate + estrogen + chem_dose + chem_dose:estrogen, reduced = ~ biological_replicate + estrogen + chem_dose)
...
```{r bbp estrogen int}
BBP_Estrogen_int <- findDEG(chem = "BBP", interaction = TRUE)
...
```{r bpa estrogen int}
BPA_Estrogen_int <- findDEG(chem = "BPA", interaction = TRUE)
...
```{r bq estrogen int}
BQ_Estrogen_int <- findDEG(chem = "BQ", interaction = TRUE)
...
```{r gen estrogen int}
Gen_Estrogen_int <- findDEG(chem = "Gen", interaction = TRUE)
...
PFHxA_Estrogen_int <- findDEG(chem = "PFHxA", interaction = TRUE)
...
```{r pfna estrogen int}
PFNA_Estrogen_int <- findDEG(chem = "PFNA", interaction = TRUE)
...
```{r pfoa estrogen int}
PFOA_Estrogen_int <- findDEG(chem = "PFOA", interaction = TRUE)
...
```{r tam estrogen int}
Tam_Estrogen_int <- findDEG(chem = "Tam", interaction = TRUE)
...
resKEGG <- pathwayTable(kegg.gs, dat[,2], sigGen[,2], noSigGen[,2])
head(resKEGG)
```
...
resGO <- pathwayTable(go.gs, dat[,2], sigGen[,2], noSigGen[,2])
head(resGO)
```
...
pathwayBarPlot <- function(finTab){
  ...
  p +geom_bar(stat = "identity", aes(fill = type)) + coord_flip() + 
  ggtitle("Dotplot of the pathways") + ylab('Number of genes') + xlab('Pathways') +
  theme(axis.text=element_text(size=12),axis.title=element_text(size=12,face="bold"),title=element_text(size=12,face="bold"))
}
```
...
pathwayDotPlot <- function(finTab){
  df <- fortify(finTab, showCategory = 10, split=split)
  df$GeneRatio <- finTab$OurGen/finTab$PathSize
  idx <- order(df$GeneRatio, decreasing = FALSE)
  df$PathName <- factor(df$PathName, levels=unique(df$PathName[idx]))
  ggplot(df, aes_string(x="GeneRatio", y="PathName",size="NrSigGen", color="Pval")) +
    geom_point() + scale_color_gradient(low="red", high="yellow") + ylab('Pathways')  +
    ggtitle("Dotplot of the pathways")+ xlab('Gene ratio (experimnet vs. pathways)') +
    theme(axis.text=element_text(size=12),axis.title=element_text(size=12,face="bold"),title=element_text(size=12,face="bold"))
}  
```
...
```{r, fig.width=11, fig.height=10}
Ugraph(p, type = "fancy", Vsize = 20, Vcex = 0.8, cut = 0.07, main="Breast cancer pathway")
```
...
genChar <- read.xls ("characteristics_table.xlsx", sheet = 2, header = TRUE)[,c(1,4)]
...
fin <- data.frame(matrix(ncol = 4, nrow = 14))  
for(i in 1:14){
...
colnames(fin) <- c("Characteristics", "CharactSize",
                   "NrSigGen", "Pval","adjPval")
fin <- fin[order(fin[,5], fin[,4], decreasing = F),]
```
...
df <- fortify(fin, showCategory = 10, split=split)
...
dotplot <- ggplot(df, aes_string(x="GeneRatio", y="Characteristics",size="NrSigGen", color="adjPval")) +
        geom_point() + scale_color_gradient(low="red", high="yellow") + ylab('Characteristics')  +
        ggtitle("Dotplot of the characteristics")+ xlab('Gene ratio') +
        theme(axis.text=element_text(size=12),axis.title=element_text(size=12,face="bold"),title=element_text(size=12,face="bold"))
```
